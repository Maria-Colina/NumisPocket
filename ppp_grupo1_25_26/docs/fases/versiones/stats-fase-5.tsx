...
export default function StatsScreen() {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      const conFotoRes = await db
        .select({
          total: sql<number>`count(distinct ${fotos_pieza.pieza_id})`,
        })
        .from(fotos_pieza);

      const totalPaisesRes = await db
        .select({
          total: sql<number>`count(distinct ${piezas.pais})`,
        })
        .from(piezas);

      const totalColeccionesRes = await db
        .select({
          total: sql<number>`count(*)`,
        })
        .from(colecciones);
      ...
      const tiposRes = await db
        .select({
          tipo: sql<string>`coalesce(${piezas.tipo}, 'Sin tipo')`,
          total: sql<number>`count(*)`,
        })
        .from(piezas)
        .groupBy(piezas.tipo)
      ...
      const continentesRes = await db
        .select({
          continente: sql<string>`coalesce(${colecciones.nombre}, 'Sin colección')`,
          total: sql<number>`count(*)`,
        })
        .from(piezas)
      ...
      setStats({
        ...
        tipos: tiposRes.map((item) => ({
          tipo: item.tipo,
          total: Number(item.total),
        })),
        continentes: continentesRes.map((item) => ({
  ...
  const totalTipos = useMemo(() => {
    return stats.tipos.reduce((acc, item) => acc + item.total, 0);
  }, [stats.tipos]);

  const tipoChartData = useMemo(() => {
    return stats.tipos.map((item, index) => {
      const nombre =
        item.tipo.toLowerCase() === 'moneda'
          ? 'Monedas'
          : item.tipo.toLowerCase() === 'billete'
            ? 'Billetes'
            : item.tipo;
      ...
      return {
        label: nombre,
        value: item.total,
        color: Colors.light.chartPalette[index % Colors.light.chartPalette.length],
      };
    });
  }, [stats.tipos]);
                      ...
                      <View key={`${item.continente}-${index}`} style={styles.barItem}>
                        ...
                        <View
                          style={[
                            ...
                            {
                              height,
                              backgroundColor: Colors.light.chartPalette[index % Colors.light.chartPalette.length],
                            },
                          ]}
                        />
                      </View>
              ...
              <View style={styles.pieWrap}>
                ...
                <View style={styles.legend}>
                  {tipoChartData.map((item, index) => {
                    const porcentaje =
                      totalTipos > 0 ? Math.round((item.value / totalTipos) * 100) : 0;
