...
export default function NewScreen() {
  ...
  useFocusEffect(
    ...
    }, [creando, editando, resetFormulario])
  );
  ...
  const nombreColeccionSeleccionada = useMemo(() => {
    ...
    return (
      collections.find((c) => String(c.id) === coleccionId)?.nombre ?? `Colección ${coleccionId}`
    );
...
const styles = StyleSheet.create({
  ...
  actionDeleteButton: { flex: 1, backgroundColor: '#F7DCDD', paddingVertical: 16, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  actionDeleteButtonText: { color: '#A44D56', fontSize: 15, fontWeight: '600' },
